...
DEFAULT_MODELS = {
    'run_1': PROJECT_ROOT / 'src/rl_training/logs/forklift_movebase_with_goal/run_1/td3_model',
    'run_2': PROJECT_ROOT / 'src/rl_training/logs/forklift_movebase_with_goal/run_2/td3_model',
}
...
def main():
    parser = argparse.ArgumentParser(description='Repeat independent validation for run_1/run_2 with stack restart per job.')
    parser.add_argument('--output-dir', type=str, required=True)
    parser.add_argument('--episodes', type=int, default=20)
    parser.add_argument('--repeats', type=int, default=3)
    parser.add_argument('--scenarios', type=str, default='noadv,adv')
    parser.add_argument('--models', type=str, default='run_1,run_2')
    ...
    for model_name in models:
        ...
        for scenario in scenarios:
            for repeat_idx in range(1, args.repeats + 1):
                ...
                csv_path = csv_dir / f'{model_name}__{scenario}__repeat{repeat_idx}.csv'
                stack_log = stack_log_dir / f'{model_name}__{scenario}__repeat{repeat_idx}.log'
                cmd = (
                    'source /opt/ros/noetic/setup.bash && '
    ...
    for i, job in enumerate(jobs, 1):
        ...
        proc = None
        log_f = None
        print(f"[{i}/{len(jobs)}] run {job['model_name']} scenario={job['scenario']} repeat={job['repeat_idx']}", flush=True)
        try:
    ...
    for item in summaries:
        key = (item['model_name'], item['scenario'])
        s = item['summary'] or {}
    ...
    aggregate_rows = []
    for (model_name, scenario), vals in sorted(agg.items()):
        n = len(vals)
        avg_success = sum(v['success_rate'] for v in vals) / n
        avg_collision = sum(v['collision_rate'] for v in vals) / n
        avg_reward = sum(v['avg_reward'] for v in vals) / n
        avg_timeout = sum(v['timeout_rate'] for v in vals) / n
    ...
    with open(out_dir / 'repeat_eval_aggregate.csv', 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['model_name', 'scenario', 'repeats', 'avg_success_rate', 'avg_collision_rate', 'avg_reward', 'avg_timeout_rate'])
        for row in aggregate_rows:
            writer.writerow([
                row['model_name'], row['scenario'], row['repeats'],
