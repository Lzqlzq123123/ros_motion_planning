...
本项目集成了自定义修改的 rsl_rl 库，用于强化学习训练。
...
- 位置: `third_party/rsl_rl/rsl_rl/modules/res_actor_critic.py`
...
1. 自动检测本地rsl_rl库位置 (`third_party/rsl_rl`)
2. 将其添加到Python路径
...
当其他电脑克隆此项目时，无需额外安装rsl_rl，因为：

1. 本地rsl_rl库已经包含在项目中 (`third_party/rsl_rl`)
2. 训练/评估脚本会自动使用本地版本
3. 只需要安装基础依赖即可（通过conda环境）

## 注意事项

- 请勿直接修改 `third_party/rsl_rl` 中的Git历史（已移除.git目录）
- 如需更新rsl_rl库，可以直接修改 `third_party/rsl_rl` 中的文件
